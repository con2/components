...
interface DateTimeInputProps {
  ...
  /// IANA timezone used to interpret/format the ISO instant in `defaultValue`
  /// and `dateRange`.
  defaultTimezone?: string;
}

function parseIsoToLocalDate(isoValue: string, timezone: string): Date {
  const zdt = Temporal.Instant.from(isoValue).toZonedDateTimeISO(timezone);
  return new Date(zdt.year, zdt.month - 1, zdt.day);
}

function parseIsoToTime(isoValue: string, timezone: string): string {
  const zdt = Temporal.Instant.from(isoValue).toZonedDateTimeISO(timezone);
...
export default function DateTimeInput({
  ...
  const [selectedDate, setSelectedDate] = useState<Date | undefined>(() => {
    if (defaultValue) {
      try {
        return parseIsoToLocalDate(defaultValue, defaultTimezone);
      } catch {
        return undefined;
      }
    }
  ...
  const [time, setTime] = useState<string>(() => {
    if (defaultValue) {
      try {
        return parseIsoToTime(defaultValue, defaultTimezone);
      } catch {
        return "";
      }
    }
  ...
  const defaultMonth = (() => {
    ...
    if (dateRange?.start) {
      try {
        return parseIsoToLocalDate(dateRange.start, defaultTimezone);
      } catch {
        return undefined;
      }
    }
  ...
  const rangeEnd = dateRange?.end
    ? (() => {
        try {
          return parseIsoToLocalDate(dateRange.end, defaultTimezone);
        } catch {
          return undefined;
        }
      })()
